...
def udp_flood(packet, ip_whitelist):
    if packet.haslayer(UDP) and packet.haslayer(IP):
        whitelisted_port = [53, 123, 443, 56230, 56976]
...
def dns_arp_spoof(packet, ip_whitelist):
    ...
    if packet.haslayer(DNS) and packet.haslayer(DNSRR):
        domain = packet[DNSRR].rrname.decode("utf-8")  # Extract domain name
        resolved_ip = packet[DNSRR].rdata              # Extract resolved IP

        # Check if domain is resolving to different IPs unexpectedly
        if domain in dns_records and dns_records[domain] != resolved_ip:
            print(f"[ALERT] DNS Spoofing Detected! {domain} resolved to {resolved_ip} (Previously: {dns_records[domain]})")
        else:
            dns_records[domain] = resolved_ip  # Update DNS records
...
def command_injection(packet, ip_whitelist):
    ...
    if packet.haslayer(Raw) and packet[IP].src not in ip_whitelist and packet[IP].dport not in whitelisted_port:
        ...
        patterns = [
            ...
            r"(wget\s+|curl\s+)",  # External callouts
            r"(rm\s+-rf\s+/)",  # Destructive commands
            r"(nc\s+-e\s+|bash\s+-i\s+|sh\s+-c\s+)",  # Reverse shell attempts
        ]

        for pattern in patterns:
            if re.search(pattern, payload, re.IGNORECASE):
                print(f"[ALERT] Command Injection Detected in live traffic: {payload}")
                return
